...
namespace GraphQlClient.Core
{
	...
	{
		public static string _hash = "d308802c6699128bdbad2106d94297a60673c5de479a85ee8b267d31d5f4bf89";

		public static Dictionary<string, string> values = new Dictionary<string, string>()
		{
			{ "X-Transaction-Token", _hash },
			{ "X-Platform", "NONE" },
			{ "X-Platform-Key", "" },
			{ "X-Project-ID", "4" },
			{ "X-Project-Token", "xT3RpsJMXpKWHPrTWkv3VBeHJKvCBccT" },
			{ "X-Language", "en" },
			{ "X-Player-Data", "" },
		};
	...
	public class HttpHandler
	{
		public static async Task<UnityWebRequest> PostAsync(string url, string details, string authToken = null){
			...
			foreach(string key in Headers.values.Keys)
            {
				if(Headers.values.TryGetValue(key, out string value))
                {
